module.exports = {
    ...
    execute(message, args, prefix, bot){   
        ...
        switch (args[1].toLowerCase()) {
            case "prefix":
                const reqprefix = require('../GuildSettings/prefix.json');
                const prefixfile = editJsonFile('./GuildSettings/prefix.json');
                ...
                if (addprefix === "!") {
                    if (reqprefix[message.guild.id]) {
                        prefixfile.unset(message.guild.id);
                        prefixfile.save();
                        return message.channel.send(`Prefix for this server set to default: ${addprefix}`);
                    }
                }
                prefixfile.set(message.guild.id, addprefix);     
                prefixfile.save();
